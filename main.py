...
img_bin = 255-img_bin
cv2.imwrite('./cv_inverted.png',img_bin)#Plotting the image to see the output
...
cv2.imwrite("vertical.jpg",vertical_lines)#Plot the generated image
...
cv2.imwrite("horizontal.jpg",
            horizontal_lines)  # Plot the generated image
...
arr = np.array(outer)
# arr += [53,83,82,15]
dataframe = pd.DataFrame(arr.reshape(len(arr),1))
# dataframe = pd.DataFrame(arr.reshape(len(row), countcol))
# dataframe = pd.DataFrame(arr.reshape(len(row)+1, countcol-1))
# dataframe = pd.DataFrame(arr.reshape(17, 4))
print(dataframe)
